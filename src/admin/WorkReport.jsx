...
const WorkReport = () => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans">
      {/* Header Section */}
      <div className="mb-6 flex justify-between items-center bg-white p-5 border border-gray-300 rounded shadow-sm">
        <div className="flex items-center gap-4">
          {/* BACK BUTTON */}
          <button 
            onClick={() => navigate("/admin/dashboard")}
            className="flex items-center justify-center w-8 h-8 bg-slate-800 text-white rounded hover:bg-slate-700 transition-colors"
          ...
          <div>
            <h1 className="text-xl font-black text-slate-800 uppercase tracking-tight border-l-4 border-blue-600 pl-3">
              Submission Logs
            </h1>
            <p className="text-xs text-gray-500 font-bold uppercase mt-1 ml-4">Daily Attendance & Proofs</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-400 shadow-lg overflow-hidden">
          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-slate-800 text-white uppercase tracking-wider">
                <th className="px-4 py-3 border border-slate-700 text-left">Staff Name</th>
                <th className="px-4 py-3 border border-slate-700 text-center">Date</th>
            ...
            <tbody>
              {reports.map((r) => (
                <tr key={r.id} className="border-b border-gray-300 hover:bg-blue-50 transition-colors">
                  <td className="px-4 py-3 border border-gray-300 font-bold text-slate-700 uppercase">
                    {r.name || "N/A"}
                  </td>
                  ...
                  <td className="px-4 py-3 border border-gray-300 text-center">
                    {r.photoURL ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-[4px] text-[10px] font-black uppercase transition-all inline-block shadow-sm"
                      >
                        View Photo
                      </a>
                    ) : (
                      <span className="text-gray-400 italic text-[10px]">No Photo</span>
                    )}
                  </td>
                  <td className="px-4 py-3 border border-gray-300 text-center">
                    <button
                      onClick={() => handleDelete(r.id)}
                      className="p-1.5 border border-red-500 text-red-500 hover:bg-red-500 hover:text-white rounded transition-all group"
                      title="Delete Entry"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {reports.length === 0 && (
          <div className="p-20 text-center">
            <p className="text-gray-400 font-bold uppercase tracking-widest text-sm">No Reports Found</p>
          </div>
        )}
      </div>
    </div>
